...
Entry to this mode runs the hooks on `comint-mode-hook'."
  ...
  (make-local-variable 'comint-last-output-end)
  (setq comint-last-output-end (make-marker))
  (set-marker comint-last-output-end (point-min))
...
buffer.  The hook `comint-exec-hook' is run after each exec."
  (save-excursion
    ...
    (let ((proc
      ...
      (goto-char (point-max))
      (set-marker comint-last-output-end (point))
      (set-marker (process-mark proc) (point)) ; courtesy
...
Similarly for Soar, Scheme, etc."
  ...
  (let* ((pmark comint-last-output-end)
    ...
    (funcall comint-input-sender
             (get-buffer-process (current-buffer)) input)
...
(defun comint-output-filter (process string)
  ;; First check for killed buffer.  If this function was not run
  ;; on behalf of a process, then it was specifically invoked within
  ;; a comint-mode buffer.
  (let ((oprocbuf (if process (process-buffer process) (current-buffer))))
	...
	(let ((obuf (current-buffer))
	  ...
	  (let ((buffer-read-only nil)
		...
		(ostart nil))
	    (widen)
	    (goto-char comint-last-output-end)
	    (setq ostart (point))
	    (if (<= (point) opoint)
...
This function should be a pre-command hook."
  (if (and comint-scroll-to-bottom-on-input
	...
	(if (< (point) comint-last-output-end)
	    (if (eq scroll 'this)
		(goto-char (point-max))
	      (walk-windows
         ...
         (function
          (lambda (window)
            (if (eq (window-buffer window) current)
                (progn
                  (select-window window)
                  (if (and (< (point) comint-last-output-end)
                           (or (eq scroll t) (eq scroll 'all)
...
(defun comint-after-pmark-p ()
  ...
  (<= (marker-position comint-last-output-end) (point)))
...
\\[view-lossage]."
  ...
  (cond ((stringp str)
         (funcall comint-input-sender (get-buffer-process (current-buffer)) str))
        (t
...
Does not delete the prompt."
  ...
  (let ((replacement nil))
    (save-excursion
      (let ((pmark (progn (goto-char comint-last-output-end)
...
Sends an EOF only if point is at the end of the buffer and there is no input."
  ...
  (let ((proc (get-buffer-process (current-buffer))))
    (if (and (eobp) proc (= (point) (marker-position comint-last-output-end)))
	(process-send-eof)
      (delete-char arg))))
...
(defun comint-set-process-mark ()
  "Set the process mark at point."  (interactive)
  (set-marker comint-last-output-end (point))
